...
def get_runs(limit=40000, compress_attributes=True, **kwargs):
    ...
    if resource_count > limit:
        print('ALERT: The specific run registry api request returns more runs than the limit({}), consider passing a greater limit to get_runs(limit=number) to get the whole result.'.format(limit))
    if resource_count > 10000:
        print('WARNING: fetching more than 10,000 runs from run registry. you probably want to pass a filter into get_runs, or else this will take a while.')
    if resource_count > 20000 and 'filter' not in kwargs:
        print('ERROR: For run registry queries that retrieve more than 20,000 runs, you must pass a filter into get_runs, an empty filter get_runs(filter={}) works')
        return None
...
def get_datasets(limit=40000, compress_attributes=True, **kwargs):
    ...
    initial_response = _get_page(
        url=url, data_type='datasets', page=0, **kwargs)
    if 'err' in initial_response:
        raise ValueError(initial_response['err'])
    ...
    if resource_count > limit:
        print('ALERT: The specific api request returns more datasets than the limit({}), consider passing a greater limit to get_datasets(limit=number) to get the whole result.'.format(limit))
    if resource_count > 10000:
        print('WARNING: fetching more than 10,000 datasets. you probably want to pass a filter into get_datasets, or else this will take a while.')
    if resource_count > 20000 and 'filter' not in kwargs:
        print('ERROR: For queries that retrieve more than 20,000 datasets, you must pass a filter into get_datasets, an empty filter get_datasets(filter={}) works')
        return None
    for page_number in range(1, page_count):
...
def get_lumisections(run_number, dataset_name='online', **kwargs):
    ...
    return _get_lumisection_helper(url, run_number, dataset_name)
...
def get_oms_lumisections(run_number, **kwargs):
    ...
    return _get_lumisection_helper(url, run_number, dataset_name='online')
